...
public class EditIssueActivity extends Activity implements
	...
	private static String url_edit_issue = "http://10.0.2.2/android_connect/edit_issue.php";
	// users JSON array
	...
	class LoadAllProjects extends AsyncTask<String, String, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = new ProgressDialog(EditIssueActivity.this);
			...
			try {
				...
				if (success == 1) {
					...
					for (int i = 0; i < projects.length(); i++) {
						...
						projectNameList.remove("choose project");
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position,
		...
		case R.id.projectnamespinner:
			...
			issue.getProject().setName(parent.getItemAtPosition(position).toString());
			break;
		case R.id.spinner:
			User user = new User();
			issue.setAssignTo(user);
			issue.getAssignTo().setUserName(parent.getItemAtPosition(position).toString());
	...
	class EditIssue extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... args) {
			...
			params.add(new BasicNameValuePair("reproducibility",
					issue.getReproducibility()));
			...
			JSONObject js = jParser.makeHttpRequest(url_edit_issue, "POST",
					params);
